...
public class GreaterTransmutation extends InventorySpell {

	{
		image = ItemSpriteSheet.GREATER_TRANSMUTATION;
	...
	protected boolean usableOnItem(Item item) {
		...
		} else if (item instanceof AncientRune) {
			return true;
	...
	public static Item changeItem(Item item) {
		if (item instanceof MagesStaff) {
			...
			return changeAncientRune((AncientRune) item);
	...
	private static AncientRune changeAncientRune(AncientRune rune) {
		AncientRune result;
		int runeNum = Random.chances(AncientRune.probabilities);

		switch (runeNum) {
			case 0:
				result = new ContravariantAncientRune();
				break;
			case 1:
				result = new CovariantAncientRune();
				break;
			case 2:
				result = new NablaAncientRune();
				break;
			case 3:
				result = new FieldOperatorAncientRune();
				break;
			case 4:
		...
		//repeat if rune unchanged
		if (rune.getClass().equals(result.getClass())) {
			result = changeAncientRune(result);
		}
	...
	public int value() {
		...
		return (50 + 15) * quantity;
	}

	public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {

		{
			inputs = new Class[] { StoneOfAugmentation.class, ScrollOfTransmutation.class, ArcaneCatalyst.class };
			inQuantity = new int[] { 1, 1, 1 };

			cost = 4;

			output = GreaterTransmutation.class;
			outQuantity = 1;
		}
